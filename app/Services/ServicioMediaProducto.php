...
{
    /**
     * Fuente para generar embeddings Voyage: disco local primero (no depende de ngrok).
     */
    public function resolveEmbeddingSource(?ProductVariant $variant): ?string
    {
        if (! $variant) {
            return null;
        }

        if (! empty($variant->image_path) && Storage::disk('public')->exists($variant->image_path)) {
        ...
        if (! empty($variant->image_url)) {
            ...
            $relative = ltrim($variant->image_url, '/');
            if (Storage::disk('public')->exists($relative)) {
                return $relative;
            }
        }

        return $this->resolvePublicUrl($variant);
    }
