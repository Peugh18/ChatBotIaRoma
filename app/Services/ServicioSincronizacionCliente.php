...
class ServicioSincronizacionCliente
{
    /**
    ...
    public function syncFromConversationContext(ConversationState $state): void
    {
        ...
        if (!$customer) {
            Log::warning('ServicioSincronizacionCliente: No customer found for state', [
        ...
        if (!empty($ctx['card_full_name']) && empty($customer->name)) {
            ...
            $needsUpdate = true;
            Log::info('ServicioSincronizacionCliente: Syncing card_full_name to customer.name', [
                'phone' => $state->phone_number,
        ...
        if (!empty($ctx['card_email']) && empty($customer->email)) {
            ...
            $needsUpdate = true;
            Log::info('ServicioSincronizacionCliente: Syncing card_email to customer.email', [
                'phone' => $state->phone_number,
        ...
        if (!empty($ctx['ship_full_name']) && empty($customer->name)) {
            ...
            $needsUpdate = true;
            Log::info('ServicioSincronizacionCliente: Syncing ship_full_name to customer.name', [
                'phone' => $state->phone_number,
        ...
        if (!empty($ctx['card_phone']) && empty($customer->alternate_phone)) {
            ...
            $needsUpdate = true;
            Log::info('ServicioSincronizacionCliente: Syncing card_phone to customer.alternate_phone', [
                'phone' => $state->phone_number,
        ...
        if (!empty($ctx['ship_phone']) && empty($customer->alternate_phone)) {
            ...
            $needsUpdate = true;
            Log::info('ServicioSincronizacionCliente: Syncing ship_phone to customer.alternate_phone', [
                'phone' => $state->phone_number,
        ...
        if ($needsUpdate && !empty($updates)) {
            ...
            Log::info('ServicioSincronizacionCliente: Customer updated successfully', [
                'phone' => $state->phone_number,
                'updates' => $updates,
            ]);
        }
